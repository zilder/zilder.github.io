...
var Balancer = {
	...
	team1_value: 0,         // output team 1 value
	team2_value: 0,         // output team 2 value
	...
	adjust_sr: false,
	adjust_sr_by_class: {},
    // exponential SR transform base (disabled if 0)
	exp_base: 0,
	...
	buildRandomBalanceVariantRolelock: function() {
		...
		this.calcObjectiveFunctionRoleLock( false, true );
	...
	// objective function is a measure of balance for combination 
	// smaller value indicates better balanced combination, 0 = perfect balance
	calcObjectiveFunctionRoleLock: function( print_debug=false, store_teams_sr=false ) {
		...
		if ( store_teams_sr ) {
			this.team1_value = team1_sr;
			this.team2_value = team2_sr;
		}
	...
	calcPlayerSRRoleLock: function ( player, class_name ) {
		...
		if ( this.exp_base > 0 ) {
			...
			player_sr = Math.pow(this.exp_base, player_sr/1000);

			// normalize
			var min = 1; 
			var max = Math.pow(this.exp_base, 5000/1000);
			player_sr = (player_sr - min) / (max - min);

			// upscale
			player_sr = player_sr * 10000;
		}
